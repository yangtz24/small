...
public class OrderConfigServiceImpl implements OrderConfigService {
    ...
    public Example createExample(OrderConfig orderConfig){
        ...
        if(orderConfig!=null){
            ...
            // 自动收货（天）
            if(!StringUtils.isEmpty(orderConfig.getAutoTake())){
                    criteria.andEqualTo("takeTimeout",orderConfig.getAutoTake());
            }
            // 售后期限
            if(!StringUtils.isEmpty(orderConfig.getServiceDeadline())){
                    criteria.andEqualTo("serviceTimeout",orderConfig.getServiceDeadline());
            }
            // 自动五星好评
            if(!StringUtils.isEmpty(orderConfig.getAutoComment())){
                    criteria.andEqualTo("commentTimeout",orderConfig.getAutoComment());
            }
        }
        return example;
    }
